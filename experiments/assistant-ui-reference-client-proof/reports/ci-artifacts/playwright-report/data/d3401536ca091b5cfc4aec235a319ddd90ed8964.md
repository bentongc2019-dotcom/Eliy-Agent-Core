...
- Name: assistant-ui-reference-client.spec.ts >> assistant-ui hosted browser reference proof >> Test A - Chat / Streaming
- Location: tests/assistant-ui-reference-client.spec.ts:257:3
...
Locator: getByText(/assistant-ui external-store runtime is active/)
Expected: visible
Error: strict mode violation: getByText(/assistant-ui external-store runtime is active/) resolved to 2 elements:
    1) <p class="message-text">Local mock stream: assistant-ui external-store ru…</p> aka getByText('Local mock stream: assistant-')
    2) <code>stream_delta: assistant-ui external-store runtime…</code> aka getByText('stream_delta: assistant-ui')

Call log:
  - Expect "toBeVisible" with timeout 8000ms
  - waiting for getByText(/assistant-ui external-store runtime is active/)
  ...
  - generic [ref=e9]:
    - generic [ref=e10]:
      - generic [ref=e11]:
        ...
        - generic [ref=e14]:
          - paragraph [ref=e15]: "Local mock stream: assistant-ui external-store runtime is active; structured tool approval is pending;"
          - generic [ref=e16]:
            - generic [ref=e17]:
    ...
    - complementary [ref=e36]:
      ...
      - list [ref=e47]:
        ...
        - listitem [ref=e60]:
          - code [ref=e61]: "stream_delta: structured tool approval is pending;"
```
...
> 264 |       await expect(page.getByText(/assistant-ui external-store runtime is active/)).toBeVisible();
  ...
  349 |
  350 |     await recordResult(page, testName, evidence, "No prior human decision is replayed during interrupt/resume.", "test-f-interrupt-resume", async () => {
  351 |       await openProof(page);
  352 |       await page.getByTestId("interrupt-button").click();
  353 |       await expect(page.getByTestId("run-state")).toContainText("interrupted");
  354 |       await expect.poll(() => eventCount(page, "run_interrupted")).toBe(1);
  355 |       await page.getByTestId("resume-button").click();
  356 |       await expect(page.getByTestId("run-state")).toContainText("running");
  357 |       await expect.poll(async () => (await eventCount(page, "run_resumed")) >= 1).toBeTruthy();
  358 |       await expect.poll(() => eventCount(page, "human_approved")).toBe(0);
  359 |       await expect.poll(() => eventCount(page, "human_denied")).toBe(0);
  360 |       await expect.poll(() => eventCount(page, "human_modified")).toBe(0);
  361 |     });
  362 |   });
  363 |
  364 |   test("Test G - Artifact", async ({ page }) => {
```